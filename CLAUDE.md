...
Reference images from the original design session are in `project/uploads/`. A running-build reference (per-station state, archive panel, tweaks overlay) is captured in `project/prototype-reference.md` with placeholder image paths under `project/uploads/prototype-*.png` — read that file when reasoning about the live UI.
